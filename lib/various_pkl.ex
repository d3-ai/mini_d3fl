defmodule VariousPkl do
  use MiniD3fl.Aliases

  def measure(node_num, packetloss, name \\ :cifar10) when is_atom(name) do
    start = System.monotonic_time(:second)
    data_directory_path = MockHelper.prepare_data_directory!(node_num, name, "Various_pkl#{packetloss * 100}%")
    # Mockのスタート

    inner_start(data_directory_path, node_num, name, packetloss)
  ...
  def inner_start(data_directory_path, node_num, name, packetloss) do
    %{job_controller_id: job_controller_id, queue: queue} = setup_num(node_num, data_directory_path, name, packetloss)
    job_executor_id = 0
  ...
  def setup_num(num, data_dir_path, name, packetloss) do
    ...
    queue = for i <- 1..num, reduce: queue do
      acc_queue ->
        ...
        for j <- 1..num do
          if i != j do
            {:ok, _channel_pid} = setup_channel(i, j, packetloss)
          else
            setup_channel(i, i, 1)
          end
        end
