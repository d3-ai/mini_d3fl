defmodule ChangePkl do
  use MiniD3fl.Aliases

  def measure(node_num, name \\ :mnist) when is_atom(name) do
    start = System.monotonic_time(:second)
    data_directory_path = MockHelper.prepare_data_directory!(node_num, name, "Change_pkl")
    # Mockのスタート
  ...
  def setup_num(num, data_dir_path, name) do
    ...
    queue = for i <- 1..num, reduce: queue do
      acc_queue ->
        ...
        for j <- 1..num do
          if i != j do
            {:ok, _channel_pid} = setup_channel(i, j, 1)
          else
            setup_channel(i, i, 1)
          end
        end
    ...
    queue = Enum.reduce(1..num, queue, fn from, from_queue ->
      Enum.reduce(1..num, from_queue, fn to, to_queue ->
        to_queue
